...
ngx_ssl_fingerprint_append_uint8(unsigned char *dst, uint8_t n)
{
    if (n < 10) {
        dst[0] = n + '0';
        dst++;
...
static inline unsigned char *
ngx_ssl_fingerprint_append_uint32(unsigned char *dst, uint32_t n)
{
    if (n < 10) {
        dst[0] = n + '0';
        dst++;
...
int ngx_http2_fingerprint(ngx_connection_t *c, ngx_http_v2_connection_t *h2c)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_EVENT, c->log, 0, "ngx_http2_fingerprint: "
                  "alloc bytes: %d", n);

    /* setting */
    for (i = 0; i < h2c->fp_settings.len; i += 5) {
        pstr = ngx_ssl_fingerprint_append_uint8(pstr,
                                                h2c->fp_settings.data[i]);
        *pstr++ = ':';
        pstr = ngx_ssl_fingerprint_append_uint32(pstr,
                                *(uint32_t *) (h2c->fp_settings.data + i + 1));
        *pstr++ = ';';
    }
    ...
    *(pstr-1) = '|';
    ...
    /* windows update */
    pstr = ngx_ssl_fingerprint_append_uint32(pstr, h2c->fp_windowupdate);
    *pstr++ = '|';

    /* priorities */
    for (i = 0; i < h2c->fp_priorities.len; i += 4) {
        pstr = ngx_ssl_fingerprint_append_uint8(pstr,
                                                h2c->fp_priorities.data[i]);
        *pstr++ = ':';
        pstr = ngx_ssl_fingerprint_append_uint8(pstr,
                                               h2c->fp_priorities.data[i + 1]);
        *pstr++ = ':';
        pstr = ngx_ssl_fingerprint_append_uint8(pstr,
                                               h2c->fp_priorities.data[i + 2]);
        *pstr++ = ':';
        pstr = ngx_ssl_fingerprint_append_uint16(pstr,
                                (uint16_t) h2c->fp_priorities.data[i + 3] + 1);
        *pstr++ = ',';
    }
